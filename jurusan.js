        ...
        for (i = 0; i < Object.keys(this.data).length; i++){
            ...
            if (value.name){
                ...
                for (j = 0; j < words.length; j++){
                    ...
                    if (j < words.length - 1){
                        if (!this.data[slug]) this.data[slug] = {};
                    ...
                    if (!this.data[initials]) this.data[initials] = {};
...
//         
        ...
        if (this.data[terms]){
            ...
            if (this.data[terms].children) for (i = 0; i < this.data[terms].children.length && limit > 0; i++){
                ...
                var subres = this.search(this.data[terms].children[i], { limit: limit, onlyExact: true });
                limit -= subres.length;
            ...
            if (this.data[terms].didYouMean) for (i = 0; i < this.data[terms].didYouMean.length && limit > 0; i++){
                ...
                var subres = this.search(this.data[terms].didYouMean[i], { limit: limit, onlyExact: true });
                limit -= subres.length;
                res = res.concat(subres);
            }
            ...
            if (this.data[terms].seeAlso) for (i = 0; i < this.data[terms].seeAlso.length && limit > 0; i++){
                ...
                var subres = this.search(this.data[terms].seeAlso[i], { limit: limit, onlyExact: true });
                limit -= subres.length;
                res = res.concat(subres);
            }
        }
        ...
        if (!onlyExact){
            ...
            for (i = 0; i < split.length - 1 && limit > 0; i++){
                // Remove the last n characters on tempTerms
                temp = temp.slice(0, -(1 + split[i+1].length));
                
                // Perform an exact search
                var subres = this.search(temp, { limit: limit, onlyExact: true });
                limit -= subres.length;
                res.concat(subres);
            }
        }
        
        // Limit the results into the specified limit
        res = res.slice(0, originalLimit);
        return res;
    }
